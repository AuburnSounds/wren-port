...
// Generate the enum table for opcodes
mixin (() {
    string str = "enum Code {";
    static foreach(op; WREN_OPCODE_FULL_TABLE) {
        str ~= "CODE_" ~ op.name ~ ", ";
    }
    str ~= "}";
    return str;
...
mixin (() {
    import std.conv : to;
    string str = "static immutable stackEffects = [";
    static foreach(op; WREN_OPCODE_FULL_TABLE) {
        str ~= to!string(op.stackEffects) ~ ", ";
    }
    str ~= "];";
    return str;
}());